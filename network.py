...
class SimpleForward:
    def __init__(self, layers, size):
        ...
        for i in range(layers-1):
            self.bias.append(np.random.random(size)-.5)
            self.weights.append(np.random.random((size, size))-.5)
        ...
        for i in range(self.iterations):
            ...
            for set in trainingSet:
                ...
                for j in range(-2, -self.layers-1, -1):
                    ...
                    g.insert(0, d*np.transpose(self.weights[j+1]).dot(g[0]))
    ...
    def gradient(self, gradient, output, Lambda):
        """ for each layer, we compute the gradient
            of each neurone """
        for i in range(1, self.layers):
            #print(self.mul(g, outputs[i-1]))
            self.weights[i] -= Lambda * self.mul(gradient[i], output[i-1])
            self.bias[i] -= Lambda * gradient[i]
    ...
    def save(self, path:str):
        """ Save the caracteristics of the network in a python file """
...
if __name__ == "__main__":
    ...
    for i in range(10):
        ...
        b = None
        if i % 2 == 0 or np.abs([1] - a[0]*2) < .05:
            a[1] = a[0]*2
            b = [0, 1]
        else:
            b = [1, 0]
        trainingSet.append([a, b])
    network = trainNetwork(SimpleForward, (3, 2), trainingSet, .08, 20000, [.2, .05])
    entry=" "
    network.save("./saves/w")
    while entry != "":
        entry = input("> ").split(" ")
